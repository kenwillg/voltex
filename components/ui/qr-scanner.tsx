...
import { AlertCircle, CheckCircle, ScanQrCode } from "lucide-react";
...
export function QrScanner({
  ...
  const [scanResult, setScanResult] = useState<ReturnType<typeof scanQrPayload> | null>(null);
  ...
  const statusBadgeClass =
    scanResult?.order?.status ? StatusManager.getStatusBadgeClass(scanResult.order.status) : "";
          ...
          {scanResult ? (
            <>
              <div className="flex items-center gap-2">
                ...
                <p className={`text-sm font-semibold ${scanResult.success ? "text-emerald-500" : "text-rose-500"}`}>
                  {scanResult.message}
                </p>
              </div>
              {scanResult.order ? (
                <div className="mt-4 space-y-3 rounded-2xl border border-border/60 p-4 text-sm">
                  <div>
                    ...
                    <p className="text-base font-semibold text-foreground">{scanResult.order.driverName}</p>
                    <p className="text-xs text-muted-foreground">{scanResult.order.licensePlate}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wide ${statusBadgeClass}`}>
                      {StatusManager.getStatusConfig(scanResult.order.status).label}
                    </span>
                    <span className="text-xs uppercase tracking-[0.3em] text-muted-foreground">
                      {scanResult.order.product} / {scanResult.order.plannedVolume}
                    </span>
                  </div>
                  <div className="grid gap-2 text-xs text-muted-foreground">
                    <p>Gate Entry: {scanResult.order.gate.entry ? new Date(scanResult.order.gate.entry).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" }) : "-"}</p>
                    <p>Gate Exit: {scanResult.order.gate.exit ? new Date(scanResult.order.gate.exit).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" }) : "-"}</p>
                    <p>Fuel Slot: {scanResult.order.fuel.slot}</p>
                  </div>
                </div>
              ) : (
                <p className="mt-3 text-sm text-muted-foreground">No driver data returned. Ensure the payload matches an existing order.</p>
              )}
            </>
          ) : (
            <div className="flex h-full flex-col items-center justify-center text-center text-sm text-muted-foreground">
              ...
              <p className="mt-2 text-xs">
                Paste the dummy JSON and hit <span className="font-semibold text-foreground">Scan QR</span> to view driver identity.
              </p>
            </div>
          )}
        </div>
