...
export default function GateOutPage() {
    ...
    const [currentStep, setCurrentStep] = useState<GateStep>("LOADING");
    const currentStepRef = useRef<GateStep>("LOADING");
    const [cameraReady, setCameraReady] = useState(false);
    const [cameraError, setCameraError] = useState<string | null>(null);
    const [statusMessage, setStatusMessage] = useState<string>("Menunggu kendaraan keluar...");
    ...
    useEffect(() => {
        ...
        socket.on("connect", () => {
            console.log("[gate-out] Connected to OCR server");
            setSocketStatus("connected");
            // Start streaming and set to QR mode
            socket.emit("start_stream");
            socket.emit("set_mode", { mode: "qr" });
            // Tell Python script this is an EXIT gate
            socket.emit("set_gate_direction", { direction: "exit" });
        });
        ...
        socket.on("driver_detected", (payload: any) => {
            ...
            setQrStatus(null);
            // For Gate Out, we expect them to be LOADING or GATE_IN
            setCurrentStep("GATE_OUT");
            setStatusMessage("Data ditemukan, silakan scan QR untuk keluar.");
            setDetectionMode("qr");
        ...
        socket.on("plate_unrecognized", (payload: any) => {
            setPlateStatus({
                ...
                plate: payload?.plate,
                confidence: payload?.confidence,
                message: payload?.message ?? "Plat tidak ditemukan.",
                timestamp: payload?.timestamp,
            });
            setQrStatus(null);
        ...
        socket.on("qr_valid", (payload: any) => {
            setQrStatus({
                ...
                message: payload?.message ?? "QR terverifikasi, silakan keluar.",
                timestamp: payload?.timestamp,
            });
        ...
        <div className="min-h-screen bg-black text-white">
            <div className="mx-auto flex max-w-6xl flex-col gap-8 px-5 py-8">
                <header className="flex items-center justify-between">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.4em] text-rose-500">Gate Out</p>
                        <h1 className="text-3xl font-semibold">Exit Kiosk</h1>
                    </div>
                        ...
                        <div className="space-y-4 rounded-2xl border border-white/10 bg-black/40 p-4">
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(["SCHEDULED", "GATE_IN", "LOADING", "GATE_OUT"] as GateStep[]).map((step) => {
                                        ...
                                        return (
                                            <span
                                                key={step}
                                                className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wide transition ${active ? "bg-rose-500 text-white" : "bg-white/10 text-neutral-300"
                                                    }`}
                                            >
                                                {step.replace("_", " ")}
                                            </span>
                                ...
                                <p className="text-xs text-neutral-400">
                                    Halaman ini khusus untuk Gate Out.
                                </p>
                            </div>
                        </div>
                    </div>
